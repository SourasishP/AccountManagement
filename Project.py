...
def transact(acc_no,amt,chk):
    ...
    if(chk==1):
        ...
        try:
            ...
            send_amt=float(input("Enter Amount you want to send:Rs."))
            if(send_amt<amt):
                ...
                cur.execute(sql,value)
                db_conn.commit()
                print("\t**Transaction Completed Successfully.\nYour Account has been debited with Rs.",send_amt)
                print("Your New Balance is Rs.",deduct_amt)
                choose(acc_no,amt)
            else:
        ...
        print("Please proceed for cash deposit of atleast Rs.500")
        send_amt=500
        for i in range(6):
            tran_id=tran_id+str(random.randint(0,9))
        tran_id="ONL"+tran_id
        ...
        cur.execute(sql,value)
        db_conn.commit()
        print("\t**Transaction Completed Successfully.\nYour Account has been credited with Rs.",send_amt)
        print("Your New Balance is Rs.",send_amt)
